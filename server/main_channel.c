...
#define PING_INTERVAL (10 * MILLI_SECOND)

#define CLIENT_CONNECTIVITY_TIMEOUT (30 * MILLI_SECOND)
...
static inline int64_t micro_now(void)
{
    struct timespec time;

    clock_gettime(CLOCK_MONOTONIC, &time);
    return 1000000LL * time.tv_sec + time.tv_nsec / 1000;
}
                                      ...
                                      void *message)
{
    ...
    case SPICE_MSGC_PONG: {
        ...
        roundtrip = micro_now() - ping->timestamp;
    ...
    const char *opt, int has_interval, int interval)
{
    ...
    if (!opt) {
        ...
        if (has_interval && interval > 0) {
            mcc->ping_interval = interval * MILLI_SECOND;
        }
